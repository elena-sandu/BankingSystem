...
/**
 * Implements a command to add interest to savings accounts within the banking system.
 */
public class AddInterest implements Commands {
    ...
    /**
     * The method checks if the account is a savings one
     * and updates the account balance and creates the transaction.
    ...
    public void execute(ArrayNode output) {
        ...
        if(user != null && account != null) {
            //if the account is a savings account, add the interest rate
            if(account.hasInterestRate() == 1) {
                ...
                //if the account is not found, we print an error
